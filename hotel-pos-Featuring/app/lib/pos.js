...
export async function getOrders({ limit = 100 } = {}) {
  ...
  const { data, error } = await supabase
    .from('orders')
    .select('id, order_number, customer_name, customer_phone, order_type, status, subtotal, discount_amount, tax_amount, total_amount, created_at, payment_status, branch_id, order_items(id,product_name,quantity,unit_price,line_total), payments(id,amount,method,status,reference,created_at)')
...
export async function createOrder({ customerName = 'Walk-in customer', customerPhone = '', items, paymentMethod = 'cash', orderType = 'counter', notes = '' }) {
  ...
  const { data, error } = await supabase.rpc('create_restaurant_order', {
    ...
    p_items: items.map((item) => ({ product_id: item.productId || item.id, quantity: Number(item.quantity) || 1 })),
    p_notes: notes,
  });
  if (error) throw error;
  const order = data?.[0];
