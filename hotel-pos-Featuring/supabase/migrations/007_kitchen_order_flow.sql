...
as $$
declare
  v_user_id uuid := auth.uid(); v_org_id uuid; v_order public.orders; v_item jsonb; v_product public.products; v_quantity numeric;
  v_subtotal numeric := 0; v_tax numeric := 0; v_discount numeric := 0; v_total numeric := 0;
begin
  ...
  if p_order_type not in ('dine_in','takeaway','delivery','room_service','counter') then raise exception 'Unsupported order type'; end if;
  ...
  select m.organization_id into v_org_id from public.memberships m where m.user_id=v_user_id and m.branch_id=p_branch_id and m.status='active' limit 1;
  if v_org_id is null then raise exception 'Active branch membership required'; end if;
  ...
  values(v_org_id,p_branch_id,nullif(trim(coalesce(p_customer_name,'')),''),nullif(trim(coalesce(p_customer_phone,'')),''),p_order_type,'sent_to_kitchen',case when p_payment_method='cash' then 'paid' else 'unpaid' end,v_user_id,p_notes)
  returning * into v_order;

  for v_item in select value from jsonb_array_elements(p_items) loop
    select p.* into v_product from public.products p where p.id=(v_item->>'product_id')::uuid and p.organization_id=v_org_id and p.is_active=true;
    ...
    values(v_order.id,v_product.id,v_product.name,v_quantity,v_product.price,0,round((v_product.price*v_quantity)*(case when v_product.is_taxable then v_product.tax_rate else 0 end)/100,2),round((v_product.price*v_quantity)+((v_product.price*v_quantity)*(case when v_product.is_taxable then v_product.tax_rate else 0 end)/100),2));
  ...
  update public.orders set subtotal=v_subtotal,tax_amount=v_tax,discount_amount=v_discount,total_amount=v_total,updated_at=now() where id=v_order.id;
